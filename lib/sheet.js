...
const COLUMN_ALIASES = {
    ...
    // Optional, and the same spellings the enrolment sheet takes. A row with one
    // names a student NSDC already holds, so nothing is registered for it.
    candidateId: ['candidateid', 'candidate', 'canid', 'candidatecode']
};

// Columns the upload cannot run without when it is registering students.
// `gender` is deliberately absent: NSDC derives it from namePrefix, matching the
// existing script's behaviour.
const REQUIRED_COLUMNS = ['namePrefix', 'name', 'dob', 'guardianName', 'email', 'phone', 'batchName'];

// A sheet that carries candidate IDs is naming students NSDC already holds, so
// there is nothing to register and nothing to check: only the ID, the email it
// belongs to and the batch it is for are needed. The rest are kept where they
// are given and ignored where they are not.
const ID_ONLY_REQUIRED = ['candidateId', 'email', 'batchName'];

// The shape NSDC gives a candidate ID. Checked because a mistyped one is still
// a real ID belonging to somebody else, which nothing downstream would catch.
...
export const TEMPLATE_COLUMNS = ['namePrefix', 'name', 'gender', 'dob', 'guardianName', 'email', 'phone', 'countryCode', 'batchName', 'candidateId'];
...
export const ASSESSMENT_COLUMNS = ['email', 'batchName', 'result'];
...
function validateRow(row, rowNumber) {
    ...
    // A row that gives a candidate ID is not registering anybody, so the fields
    // a registration needs are not checked. Only the three that are used stay:
    // the ID itself, the email it belongs to, and the batch it is for.
    if (row.candidateId) {
        if (!CANDIDATE_ID_RE.test(row.candidateId)) {
            errors.push(`candidateId "${row.candidateId}" is not a candidate ID — they read as CAN_ and digits`);
        }
        if (!row.email) {
        ...
        const batchProblem = checkBatchName(row.batchName);
        if (batchProblem) errors.push(batchProblem);
...
export async function parseStudentSheet(buffer, filename) {
    ...
    // A sheet with a candidateId column is read as naming students NSDC already
    // has, so the columns a registration would need are not required of it.
    const byId = 'candidateId' in mapping;
    const missing = (byId ? ID_ONLY_REQUIRED : REQUIRED_COLUMNS)
        .filter(column => !(column in mapping));
...
export async function parseAssessmentSheet(buffer, filename) {
    ...
    const missing = ASSESSMENT_COLUMNS.filter(column => !(column in mapping));
    if (missing.length > 0) {
        return {
            rows: [], errors: [], totalRows: rawRows.length, mapping, ignoredColumns: unmatched,
            headerErrors: [`Missing column(s): ${missing.join(', ')}. Found: ${headers.filter(Boolean).join(', ')}`]
    ...
    rawRows.forEach((raw, index) => {
        ...
        if (!row.email && !row.batchName && !row.result) return;
