...
export const BATCH_IMPORT_COLUMNS = ['batchName', 'batchId'];
...
/**
 * Reads a sheet of batches that already exist on NSDC — a name and the ID NSDC
 * gave it. Used when a batch was not created through this portal: the batch
 * creation API was unavailable for six months, and those batches were made on
 * the NSDC site instead, so their IDs have to be brought in by hand before
 * anyone can be enrolled into them.
 */
export async function parseBatchImportSheet(buffer, filename) {
    const isExcel = /\.xlsx?$/i.test(filename);
    const { headers, rows: rawRows } = isExcel ? await readXlsx(buffer) : readCsv(buffer);
    ...
    const NAME_ALIASES = ['batchname', 'batch', 'name'];
    const ID_ALIASES = ['batchid', 'id', 'nsdcbatchid'];
    ...
    for (const header of headers) {
        const normalized = normalizeHeader(header);
        if (!normalized) continue;
        if (NAME_ALIASES.includes(normalized) && !mapping.batchName) mapping.batchName = header;
        else if (ID_ALIASES.includes(normalized) && !mapping.batchId) mapping.batchId = header;
        else unmatched.push(header);
    }

    const missing = BATCH_IMPORT_COLUMNS.filter(column => !(column in mapping));
    if (missing.length > 0) {
        return {
            rows: [], errors: [], totalRows: rawRows.length, mapping, ignoredColumns: unmatched,
            headerErrors: [`Missing column(s): ${missing.join(', ')}. Found: ${headers.filter(Boolean).join(', ')}`]
    ...
    rawRows.forEach((raw, index) => {
        ...
        if (!row.batchName && !row.batchId) return;

        const rowErrors = [];
        const batchNameProblem = checkBatchName(row.batchName);
        if (batchNameProblem) rowErrors.push(batchNameProblem);
        if (!row.batchId) {
            rowErrors.push('batchId is empty');
        } else if (!/^\d+$/.test(row.batchId)) {
            rowErrors.push(`batchId "${row.batchId}" must be the number NSDC issued, e.g. 3931424`);
        } else if (seen.has(row.batchId)) {
            rowErrors.push(`batchId ${row.batchId} appears more than once in this sheet`);
        }
        ...
        if (rowErrors.length > 0) {
            ...
            row.batchId = Number(row.batchId);
            row.rowNumber = rowNumber;
            rows.push(row);
        }
    });
