...
async function createCandidate(client, student, csrfToken, authToken) {
    // A row that already carries a candidate ID names a student NSDC holds
    // already. Registering them again would either be refused or, worse, make a
    // second candidate, so nothing is sent: the ID is taken as given and only
    // recorded here, which is what the portal was missing for them.
    if (student.candidateId) {
        return { candidateId: String(student.candidateId).toUpperCase(), status: 'EXISTING' };
    }
    ...
    const endpoint = USER_SERVICE_URL + '/api/user/v1/register/Candidate/v1';
    ...
    if (response.ok) {
        let body = {};
        try { body = JSON.parse(raw); } catch { /* handled by the candidateId check */ }
        if (!body.candidateId) {
            const error = new Error('NSDC accepted the row but returned no candidateId');
    ...
    const message = errorBody.message || raw.slice(0, 300) || response.statusText;
 ...
 * each row. `duplicates` counts both the students NSDC already had and the rows
 * that arrived with a candidate ID of their own.
    ...
    for (const student of students) {
        ...
        while (!done) {
            try {
                ...
                // Only a row that actually registered somebody counts as created.
                // A duplicate and a row that came in with its own ID both name a
                // student NSDC already had.
                if (outcome.status === 'NEW') created++; else duplicates++;
                done = true;
            } catch (err) {
                attempt++;
                ...
                if (err.httpStatus === 412 && attempt === 1) {
                    try {
                        ...
                        const reAuthRow = { ...student, candidateId: '', status: 'FAILED', error: `Re-authentication failed: ${reAuthError.message}`, call: reAuthError.call || describeCall({ endpoint: USER_SERVICE_URL + '/api/user/v1/login', responseBody: reAuthError.message }), attempts: attempt };
                        results.push(reAuthRow);
                        if (onResult) await onResult(reAuthRow);
                        failed++;
                        done = true;
                        break;
                    }
                ...
                const failedRow = { ...student, candidateId: '', status: 'FAILED', error: err.message, call: err.call || null, attempts: attempt };
