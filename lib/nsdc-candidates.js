...
async function authenticate(client, userName, password) {
    ...
    if (!response.ok) {
        // What NSDC said, not what HTTP calls the number. "Bad Request" is the
        // protocol's own phrase for 400 and carries nothing; the body is where
        // a rejected password is told apart from a missing field, and this is
        // the one call the failures page cannot explain without it.
        const raw = (await response.text().catch(() => '')).trim();
        let said = raw;
        try {
            const body = JSON.parse(raw);
            said = body.message || body.error || raw;
        } catch { /* plain text answer, use it as-is */ }

        const error = new Error(`Authentication failed: ${response.status} ${said || response.statusText}`);
        error.httpStatus = response.status;
        error.call = describeCall({
            endpoint: USER_SERVICE_URL + '/api/user/v1/login',
            status: response.status,
            responseBody: raw || response.statusText
        });
        throw error;
    }
