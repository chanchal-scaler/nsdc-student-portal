...
async function enrollGroup(client, batchId, candidateIds, csrfToken, authToken) {
    ...
    const request = client(USER_SERVICE_URL + '/api/thirdparty/v1/enroll/Candidate', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-Csrf-Token': csrfToken,
        ...
        body: JSON.stringify(buildEnrollmentPayload(batchId, candidateIds))
    });
 ...
 */
export async function enrollCandidates({ userName, password, groups, onProgress }) {
