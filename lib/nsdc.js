...
async function fetchStudentPage(client, pageNo, csrfToken, authToken, tpId, filters = {}) {
    ...
    const endpoint = `${apiUrl}?${params}`;
    const payload = { createdBy: [tpId], tpId };
 ...
 * only where the ID is one we asked about.
 *
 * Every candidate is read, unfiltered. `isEnrolledToBatch=Yes` looks like the
 * filter for this and would page through less, but the flag it names cannot be
 * trusted: in a download of 36,159 candidates, 14,327 are in at least one batch
 * and 8,902 of those still say "No" — including all 119 of one batch this
 * portal created. Narrowing on it would risk silently hiding most of the
 * enrolments this read exists to verify, so the slower full read is the honest
 * one.
 *
...
export async function fetchCandidatesForBatches({ userName, password, tpId, batchIds, onProgress }) {
    ...
    // A membership in a batch we did not ask about is skipped silently, which
    // makes "0 in these batches" impossible to tell apart from "read nothing".
    // Counting the skips, and keeping a few of the IDs, says which it was.
    ...
    const report = () => {
        if (onProgress) {
            onProgress({
                ...
                membershipsSeen,
                membershipsElsewhere,
                otherBatchIds: [...otherBatchIds].slice(0, 5),
    ...
    while (pageNo <= maxPages) {
        ...
        for (const candidate of page.data) {
            ...
            for (const membership of candidate.batches || []) {
                ...
                if (!batchId) continue;
                membershipsSeen++;
                if (wanted.size > 0 && !wanted.has(batchId)) {
                    membershipsElsewhere++;
                    if (otherBatchIds.size < 5) otherBatchIds.add(batchId);
                    continue;
                }
