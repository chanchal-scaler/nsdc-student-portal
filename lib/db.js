...
const pool = isEnabled
    ? new Pool({
        ...
        // A local Postgres has no SSL at all, so sslmode=disable turns it off
        // and lets the portal be run against a throwaway database.
...
/**
 * The other direction: what this portal knows about a candidate ID typed in by
 * hand. Used to show whose ID it is before anybody is enrolled — NSDC accepts a
 * wrong-but-real ID without complaint, so a typo is only catchable here. Null
 * means unknown to the portal, which is not an error: a student registered
 * outside it still enrols.
 */
export async function findCandidateById(candidateId) {
    if (!pool) return null;
    const { rows } = await pool.query(
        `SELECT candidate_id, email, name FROM candidates WHERE candidate_id = $1`,
        [candidateId]
    );
    return rows[0] || null;
}

/** Same, for a batch ID: the name it was created under, if this portal created it. */
export async function findBatchById(batchId) {
    if (!pool) return null;
    const { rows } = await pool.query(
        `SELECT batch_id, batch_name FROM batches WHERE batch_id = $1`,
        [batchId]
    );
...
/**
 * Everyone enrolled in a batch whose results have not been submitted yet, with
 * the IDs a completion row needs. Read from here rather than kept as a file
 * because the disk does not survive a restart, and results are often filled in
 * days after the enrolment that produced them.
 */
export async function pendingCompletions() {
    if (!pool) return [];
    const { rows } = await pool.query(
        `SELECT e.candidate_id, e.batch_id, e.batch_name, c.email
         FROM enrollments e
         LEFT JOIN candidates c ON c.candidate_id = e.candidate_id
         WHERE e.status <> 'COMPLETED'
         ORDER BY e.batch_name, e.candidate_id`
    );
    return rows.map(r => ({
        candidateId: r.candidate_id,
...
export async function enrolmentHistory({ limit = 10, offset = 0 } = {}) {
    ...
    const [students, byIdOnly, emptyBatches, unnamed, stoppedRuns, nsdc] = await Promise.all([
        pool.query(
        ...
        // Learners enrolled by candidate ID who were never uploaded from here:
        // they were registered with NSDC before this portal existed, so there is
        // no row in candidates for the query above to start from, and the batch
        // they are in read as empty. Their name and email are genuinely unknown
        // here — the enrolment sheet carried an ID and nothing else.
        pool.query(
            `SELECT b.batch_name, b.batch_id, b.created_at AS batch_created_at,
                    e.candidate_id, e.status AS enrolment_status, e.enrolled_at
             FROM enrollments e
             JOIN batches b ON b.batch_id = e.batch_id
             WHERE NOT EXISTS (
                 SELECT 1 FROM candidates c
                 WHERE c.candidate_id = e.candidate_id
                   AND lower(c.batch_name) = lower(b.batch_name))`),
    ...
    for (const row of byIdOnly.rows) {
        ensure(row.batch_name, row.batch_id, row.batch_created_at).students.push({
            candidateId: row.candidate_id,
            name: null,
            email: null,
            ...
            return {
                ...
                students: [...batch.students]
                    ...
                    .sort((a, b) =>
                        ...
                        String(a.name || a.email || a.candidateId).localeCompare(String(b.name || b.email || b.candidateId))),
