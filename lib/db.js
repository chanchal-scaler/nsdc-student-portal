...
/**
 * The most recent rows a page has stored, newest first, with a total. Every
 * page shows what it has already done — otherwise the only record of a run is
 * a result CSV someone has to have kept.
 */
export async function recentCandidates(limit = 25) {
    if (!pool) return { rows: [], total: 0 };
    const [list, count] = await Promise.all([
        pool.query(
            `SELECT candidate_id, email, name, batch_name, status, source_file, created_at
             FROM candidates ORDER BY id DESC LIMIT $1`, [limit]),
        pool.query('SELECT count(*)::int AS n FROM candidates')
    ]);
    return { rows: list.rows, total: count.rows[0].n };
}

export async function recentBatches(limit = 25) {
    if (!pool) return { rows: [], total: 0 };
    const [list, count] = await Promise.all([
        pool.query(
            `SELECT b.batch_id, b.batch_name, b.source_file, b.created_at,
                    (SELECT count(*)::int FROM candidates c
                      WHERE lower(c.batch_name) = lower(b.batch_name)) AS students
             FROM batches b ORDER BY b.id DESC LIMIT $1`, [limit]),
        pool.query('SELECT count(*)::int AS n FROM batches')
    ]);
    return { rows: list.rows.map(r => ({ ...r, batch_id: Number(r.batch_id) })), total: count.rows[0].n };
}

/**
 * Enrolments, optionally narrowed to those whose results are in.
 *
 * A completed student is still an enrolled one — completion is a later state of
 * the same row — so the enrolment page counts both, and only the completion
 * page narrows to COMPLETED.
 */
export async function recentEnrollments({ completedOnly = false, limit = 25 } = {}) {
    if (!pool) return { rows: [], total: 0 };
    const where = completedOnly ? `WHERE e.status = 'COMPLETED'` : '';
    const countWhere = completedOnly ? `WHERE status = 'COMPLETED'` : '';
    const [list, count] = await Promise.all([
        pool.query(
            `SELECT e.candidate_id, c.email, e.batch_name, e.batch_id, e.status,
                    e.source_file, e.enrolled_at
             FROM enrollments e
             LEFT JOIN candidates c ON c.candidate_id = e.candidate_id
             ${where}
             ORDER BY e.id DESC LIMIT $1`, [limit]),
        pool.query(`SELECT count(*)::int AS n FROM enrollments ${countWhere}`)
    ]);
    return { rows: list.rows.map(r => ({ ...r, batch_id: Number(r.batch_id) })), total: count.rows[0].n };
}
