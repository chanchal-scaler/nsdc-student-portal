...
// Railway's Postgres presents a self-signed cert on the public proxy, so SSL is
// on but unverified. A database on this machine has no certificate at all and
// refuses the handshake outright, which used to make the portal impossible to
// run against a local one — and the failures page, which only records anything
// when a database is attached, impossible to try before deploying it.
const isLocalDatabase = /^(postgres(ql)?:\/\/)?[^@]*@?(localhost|127\.0\.0\.1|\[::1\])[:\/]/.test(connectionString || '') ||
