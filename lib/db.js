...
/**
 * A one-line answer to "where did we get to last time".
 *
 * Uploads happen every few months, not every month, so someone coming back
 * needs to see which months are already done before they can tell what is
 * left. Batch names carry the month, so the batches covered are the useful
 * part — not the row count.
 */
export async function lastRunSummary() {
    if (!pool) return null;

    const [students, batches, enrolments] = await Promise.all([
        pool.query(
            `SELECT count(*)::int AS total, max(created_at) AS last_at,
                    (SELECT array_agg(DISTINCT batch_name)
                       FROM candidates
                      WHERE batch_name IS NOT NULL AND batch_name <> '') AS batches
             FROM candidates`),
        pool.query(
            `SELECT count(*)::int AS total, max(created_at) AS last_at,
                    (SELECT batch_name FROM batches ORDER BY id DESC LIMIT 1) AS latest
             FROM batches`),
        pool.query(
            `SELECT count(*)::int AS total, max(enrolled_at) AS last_at,
                    count(*) FILTER (WHERE status = 'COMPLETED')::int AS completed,
                    (SELECT max(enrolled_at) FROM enrollments WHERE status = 'COMPLETED') AS last_completed_at
             FROM enrollments`)
    ]);

    return {
        students: {
            total: students.rows[0].total,
            lastAt: students.rows[0].last_at,
            batches: students.rows[0].batches || []
        },
        batches: {
            total: batches.rows[0].total,
            lastAt: batches.rows[0].last_at,
            latest: batches.rows[0].latest
        },
        enrolments: {
            total: enrolments.rows[0].total,
            lastAt: enrolments.rows[0].last_at,
            completed: enrolments.rows[0].completed,
            lastCompletedAt: enrolments.rows[0].last_completed_at
        }
    };
}
