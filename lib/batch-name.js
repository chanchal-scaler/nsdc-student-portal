...
export function checkBatchName(name) {
    ...
    if (spelled && spelled !== programme) {
        return `batch name "${value}" should spell the programme "${spelled}"`;
    }
    ...
    if (YEAR_WISE_PROGRAMMES.includes(programme)) {
        ...
        return `batch name "${value}" must be a programme, a four-digit year, the course ` +
    ...
    if (parts.length !== 2) {
        ...
        return wrongShape
            ? `batch name "${value}" names a year, but ${programme} runs in monthly intakes — ` +
              `name it for its month, like "${programme} Sep26"`
            : `batch name "${value}" must be a programme and a month, like "Academy Sep26"`;
    }
