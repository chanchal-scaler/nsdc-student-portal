...
// Add a programme here when it works that way, and say so on the three upload
// pages, which name the programmes in prose rather than reading them from here.
export const YEAR_WISE_PROGRAMMES = ['SST', 'SSB'];

// The course an SST batch is for — "CS-AI", "AI-B", and whatever starts next.
// Not a fixed list: courses come and go, and a list would mean editing this
// file before a new one could be named. Letters, digits and hyphens only, so a
// code cannot carry a space and split one batch into two; case does not matter,
// since every batch lookup already compares names in lower case.
const COURSE = /^[A-Za-z][A-Za-z0-9-]{0,19}$/;
...
const YEAR_PART = /^(\d{4})(?:\s+(\S+))?\s+Year\s+([1-9])$/i;

// The course is optional because the batches named before it was used do not
// carry one, and they still have to be reachable from here.
function yearShapeOk(rest) {
    const m = YEAR_PART.exec(rest);
    if (!m) return false;
    return m[2] === undefined || COURSE.test(m[2]);
}
...
export const BATCH_NAME_HINT =
    ...
    `${YEAR_WISE_PROGRAMMES.join(' and ')} ${YEAR_WISE_PROGRAMMES.length > 1 ? 'run' : 'runs'} ` +
    `in years, so their batches read as ` +
    `<Programme> <YYYY> <Course> Year <n> — for example "SST 2023 CS-AI Year 3"; the course is ` +
    `letters, digits and hyphens, and may be left out on batches named before it was used.`;
...
export function checkBatchName(name) {
    ...
    if (YEAR_WISE_PROGRAMMES.includes(programme)) {
        ...
        if (yearShapeOk(rest)) return null;
        return `batch name "${base}" must be a programme, a four-digit year, the course ` +
            `and which year of it, like "${programme} 2023 CS-AI Year 3"`;
    }
    ...
    if (parts.length !== 2) {
        ...
        const wrongShape = PROGRAMMES.includes(programme) && yearShapeOk(parts.slice(1).join(' '));
        return wrongShape
