...
async function previewPayload() {
  ...
  let res;
  try {
    res = await fetch('/api/upload/preview', { method: 'POST', body: form });
  } catch (err) {
...
async function startUpload() {
  ...
  let res;
  try {
    res = await fetch('/api/upload/students', { method: 'POST', body: form });
  } catch (err) {
...
function showRunning(data) {
  ...
  statusEl.appendChild(document.createTextNode(
    'Registering student ' + Math.min(data.processed + 1, data.total) + ' of ' + data.total +
    ' — ' + data.created + ' new, ' + data.duplicates + ' existing, ' + data.failed + ' failed'
  ));
...
async function poll() {
  ...
  if (data.dryRun) {
    banner.style.display = 'block';
    banner.textContent = 'Test mode: rows are validated and fake candidate IDs are returned. Nothing is sent to NSDC.';
  }
